...
const useMovieStore = create((set, get) => ({
  ...
  try {
    ...
    fetchRecommendedMovies: async () => {
        set({ loading: true, error: null });
        try {
            const response = await axiosInstance.get(`/recommendation/recommended`);
            set({ currentMovie: response.data.data || response.data, loading: false });
            return response.data.data || response.data;
        } catch (error) {
            set({ error: error.message, loading: false });
    ...
    fetchWatchlist: async () => {
        ...
        try {
            ...
            const id = String(movie._id); // force to string
            watchlistMap[id] = true;
            return { ...movie, _id: id }; // also force _id to string in object
            });
