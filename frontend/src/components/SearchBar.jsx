...
const SearchBar = () => {
    ...
    useEffect(() => {
        const fetchAllMovies = async () => {
            try {
                const response = await fetchMovies(1, 1000, { genres: "", regions: "", years: "" });
                    ...
                    <div className="search-bar" role="search" aria-label="Site search">
                        ...
                        {searchTerm.trim().length > 0 &&
                            <ClearIcon
                                ...
                                onMouseDown={() => setWasInputFocused(document.activeElement === inputRef.current)}
                                onClick={clearInput}
                            />
                        }
                    ...
                    <div className="dropdown">
                        {filteredItems.length > 0 ? (
                            filteredItems.map((movie, index) => (
                                <SearchItem movie={movie} key={index} onClick={() => handleSearchItemClick(movie)} />
                                // <div
                                //     key={index}
                            ...
                            (searchTerm.trim().length > 0) && (
                                <div className="search-dropdown-item no-results">No results found</div>
                            )
