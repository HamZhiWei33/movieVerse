...
export const getRankingData = async (req, res) => {
  try {
    ...
    const movies = await Movie.find({})
      .select(
        "title posterUrl rating year genre description region duration trailerUrl"
      )
      .sort({ rating: -1 })
      .lean();
    ...
    console.log("Fetched genres ranking:", genres); // Debug log
...
export const getGenreRankings = async (req, res) => {
  try {
    ...
    const movies = await Movie.find(query)
      .select(
        "title posterUrl rating year genre description region duration _id"
      )
      .lean();
    ...
    const genres = await Genre.find({}).select("id name").lean();
